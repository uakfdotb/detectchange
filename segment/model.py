...
class Model:
	...
	def __init__(self, bn=False, size=256):
		...
		self.inputs = tf.placeholder(tf.float32, [None, size, size, 3])
		self.targets = tf.placeholder(tf.float32, [None, size/4, size/4, 1])
		...
		self.pre_outputs = self._conv_layer('pre_outputs', self.layer15, 1, 256, 1, {'activation': 'none', 'batchnorm': False}) # -> 64x64x1
		self.outputs = tf.nn.sigmoid(self.pre_outputs)

		#self.loss = tf.reduce_mean(tf.square(self.outputs - self.targets))
		self.loss = tf.nn.sigmoid_cross_entropy_with_logits(labels=self.targets, logits=self.pre_outputs)
